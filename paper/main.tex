...
\begin{table}[h]
\centering
\caption{v2 benchmark results across 6 categories (58 total episodes).}
\label{tab:v2benchmark}
\begin{tabular}{lccccccc}
\toprule
Domain & $n$ & TP & FP & TN & FN & Prec & Recall \\
\midrule
Finance & 20 & 7 & 2 & 7 & 4 & 78\% & 64\% \\
Commodities & 10 & 4 & 2 & 2 & 2 & 67\% & 67\% \\
Housing (FHFA) & 10 & 2 & 1 & 3 & 4 & 67\% & 33\% \\
Housing (Zillow) & 6 & 2 & 1 & 1 & 2 & 67\% & 50\% \\
Adversarial & 6 & 1 & 0 & 5 & 0 & 100\% & 100\% \\
Forward 2024--25 & 6 & 3 & 0 & 3 & 0 & 100\% & 100\% \\
\midrule
\textbf{Overall} & \textbf{58} & \textbf{19} & \textbf{6} & \textbf{21} & \textbf{12} & \textbf{76\%} & \textbf{61\%} \\
\bottomrule
\end{tabular}
\end{table}
...
\textbf{Limitations.} (1)~Sornette filter thresholds ($m < 0.87$, $|B| > 0.003$) were initially derived observing all 12 evaluation episodes. Post-hoc verification on the held-out test split (5 episodes, unseen during tuning) confirms generalization: precision = 100\%, recall = 33\%, 0 false positives. Time-forward validation on 2024 data (3 bubbles detected, 0 FP) further supports this, but formal cross-validated threshold selection is needed. (2)~Fraud results on synthetic data only. (3)~Overall recall = 61\%: the system misses ${\sim}$40\% of bubbles, prioritizing precision over recall. (4)~Geological ``phase transitions'' lack ground truth events. (5)~TOST equivalence confirmed only for Finance$\leftrightarrow$Commodities ($p=0.031$); cross-domain comparison with geology shows large effect size (Cohen's $d=1.66$) and requires larger samples.
